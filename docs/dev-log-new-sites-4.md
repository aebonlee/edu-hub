...
교양분야에 실용 일본어·실용 중국어, 코딩학습에 자바 마스터·파이썬 마스터 총 4개 학습사이트를 추가하여 커리큘럼을 확대한다.
...
| 파일 | 변경 내용 |
|------|----------|
| `src/config/site.js` | menuItems 드롭다운에 4개 메뉴 추가, learningSites 배열에 4개 사이트 객체 추가 |
| `src/utils/translations.js` | 한국어·영어 nav 키 4개씩 추가 (`japanese`, `chinese`, `javaMaster`, `pythonMaster`) |
...
- `npm run dev` 로컬 실행 후 navbar 드롭다운에 새 메뉴 4개 표시 확인
